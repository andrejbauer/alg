...
let does_contradict alg axiom =
  try begin
    let lc = Array.length alg.alg_const in
    let lu = Array.length alg.alg_unary in
    let lb = Array.length alg.alg_binary in
    let lp = Array.length alg.alg_predicates in
    let lr = Array.length alg.alg_relations in
    let n = alg.alg_size in

    (* Main operation tables for unary operations. *)
    let unary_arr = alg.alg_unary in
    (*
      Main operation tables for binary operations.
    *)
    let binary_arr = alg.alg_binary in

    (* Main operation tables for predicates. *)
    let pred_arr = alg.alg_predicates in
    (*
      Main operation tables for relations.
    *)
    let rel_arr = alg.alg_relations in

               
    (* Auxiliary variables for generation of unary operations. *)
    (* ******************************************************* *)
    let (unary_axioms, binary_axioms) = part_axioms axiom in
      (*
    ...
    let (unary_dos, unary_undos) = Enum_unary.get_unary_actions normal_axioms alg in
      ...
      for o=0 to lu - 1 do
        for i=0 to n-1 do
          if not (unary_dos (o,i)) then
            raise InconsistentAxioms
        done
      done ;
      ...
      *)
      ...
      let check_after_add () =
        for o=0 to lb-1 do
          for i=0 to n-1 do
            for j=0 to n-1 do
              if not (binary_dos (o,i,j) o i j) then
                raise InconsistentAxioms
            done
          done
        done in
      ...
      let cont_binary () =
        try
          ...
          if not (check ()) then raise InconsistentAxioms ;
        with Enum_binary.Contradiction -> () in
        
        cont_binary() ;
        false
  end
  with InconsistentAxioms -> true
